...
public class DatePicker extends FrameLayout {
    ...
    DatePicker(ViewGroup root, int numberPickerStyle) {
        ...
        // Hour
        mHourSpinner = (NumberPicker) inflater.inflate(R.layout.number_picker_year,
                mPickerContainer, false);
        mHourSpinner.setId(R.id.hour);
        mHourSpinner.setOnLongPressUpdateInterval(100);
        mHourSpinner.setOnValueChangedListener(onChangeListener);
        mHourSpinnerInput = NumberPickers.findEditText(mHourSpinner);

        // Minute
        mMinuteSpinner = (NumberPicker) inflater.inflate(R.layout.number_picker_year,
                mPickerContainer, false);
        mMinuteSpinner.setId(R.id.minute);
        mMinuteSpinner.setOnLongPressUpdateInterval(100);
        mMinuteSpinner.setOnValueChangedListener(onChangeListener);
        mMinuteSpinnerInput = NumberPickers.findEditText(mMinuteSpinner);
    ...
    void init(int year, int monthOfYear, int dayOfMonth, int hour,int minute,
    ...
    void updateDate(int year, int month, int dayOfMonth,int hour,int minute) {
        if (!isNewDate(year, month, dayOfMonth,hour,minute)) {
            return;
        }
    ...
    int getHour(){ return  mCurrentDate.get(Calendar.HOUR_OF_DAY);}

    int getMinute(){ return  mCurrentDate.get(Calendar.MINUTE);}
    ...
    private void reorderSpinners() {
        ...
        if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.JELLY_BEAN_MR2) {
            ...
            pattern = DateFormat.getBestDateTimePattern(Locale.getDefault(), "yyyyMMMddHHmm");
        }
    ...
    private boolean isNewDate(int year, int month, int dayOfMonth,int hour,int minute) {
        return (mCurrentDate.get(Calendar.YEAR) != year
                || mCurrentDate.get(Calendar.MONTH) != month
                || mCurrentDate.get(Calendar.DAY_OF_MONTH) != dayOfMonth
                || mCurrentDate.get(Calendar.HOUR_OF_DAY)!=hour
                || mCurrentDate.get(Calendar.MINUTE) !=minute);
    }

    private void setDate(int year, int month, int dayOfMonth,int hour,int minute) {
        mCurrentDate.set(year, month, dayOfMonth,hour,minute);
        if (mCurrentDate.before(mMinDate)) {
            mCurrentDate.setTimeInMillis(mMinDate.getTimeInMillis());
    ...
    private void updateSpinners() {
        ...
        //hour spinner range does not change base on any date

        mHourSpinner.setMinValue(mMinDate.get(Calendar.HOUR_OF_DAY));
        mHourSpinner.setMaxValue(mMaxDate.get(Calendar.HOUR_OF_DAY));
        mHourSpinner.setWrapSelectorWheel(true);


        mMinuteSpinner.setMinValue(mMinDate.get(Calendar.MINUTE));
        mMinuteSpinner.setMaxValue(mMaxDate.get(Calendar.MINUTE));
        mHourSpinner.setWrapSelectorWheel(true);
        ...
        //turning off visibility so that we can custime the layout as user needed
        if(!SpinnerDatePickerDialogBuilder.showMinute)mMinuteSpinner.setVisibility(GONE);
        if(!SpinnerDatePickerDialogBuilder.showHour)mHourSpinner.setVisibility(GONE);
        if(!SpinnerDatePickerDialogBuilder.showDay)mDaySpinner.setVisibility(GONE);
        if(!SpinnerDatePickerDialogBuilder.showMonth)mMonthSpinner.setVisibility(GONE);
        if(!SpinnerDatePickerDialogBuilder.showYear)mYearSpinner.setVisibility(GONE);
